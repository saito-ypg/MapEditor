...
void Stage::SaveStage()
{
	...
	WriteFile(
		hFile,                   //ファイルハンドル
		writeStr.str().c_str(),                  //保存するデータ（文字列）
		(DWORD)strlen(writeStr.str().c_str()),   //書き込む文字数
...
BOOL Stage::DialogProc(HWND hDlg, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(GetDlgItem(hDlg, IDC_SPIN_HEIGHT), UDM_SETRANGE, 0, (LPARAM)MAKEWORD(49, 0));//setrangeてcommctrl.hないとつかえないの
		return TRUE;
	...
	case WM_COMMAND:
		...
		if (HIWORD(wp) == EN_KILLFOCUS)//editControlの入力終わったら中身を取り出す
		{
			char buf[4];
			GetWindowText(GetDlgItem(hDlg, ID_EDIT_HEIGHT), buf, 4);
			int h = std::stoi(buf);
